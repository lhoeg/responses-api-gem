...
module ResponsesApi
  class RetrieveResponsesRequest < FormRequest
    ISO_8601_FORMAT = '%Y-%m-%dT%H:%M:%S.%L%z'.freeze
    ...
    def initialize(form_id, token: APIConfig.token, page_size: 25, page: nil, ts_since: nil, ts_until: nil, after: nil, before:nil, completed: true,
      ...
      url << "ts_since=#{ts_since.utc.strftime(ISO_8601_FORMAT)}&" unless ts_since.nil?
      url << "ts_until=#{ts_until.utc.strftime(ISO_8601_FORMAT)}&" unless ts_until.nil?
